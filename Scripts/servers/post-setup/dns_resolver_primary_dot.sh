...
# Cert used to serve DNS-over-TLS to LAN clients. Bound to the host
# principal created by ipa-client-install, so no separate service-add
# is needed. certmonger keeps it renewed automatically.
configure_dot_cert() {
...
# dns_resolver_primary.sh plus the new DoT rule.
