#!/bin/bash
# Health check for db-proxy-2.lab.internal
# Run as: sudo ./healthcheck_db_proxy_2.sh
set -uo pipefail
...
SELF_V4=10.0.0.42
PEER_V4=10.0.0.41
PEER_FQDN=db-proxy-1.lab.internal
DB1_V4=10.0.0.43
...
if [ -f "$TLS_COMBINED" ]; then
    ...
    hc_fail "combined haproxy PEM missing: $TLS_COMBINED" "re-run configure_tls_cert() from db_proxy_secondary.sh"
fi
...
if [ -f /etc/keepalived/notify.sh ] && [ -x /etc/keepalived/notify.sh ]; then
    ...
    hc_fail "keepalived notify.sh hook missing or not executable" "re-run configure_conntrackd() from db_proxy_secondary.sh; sudo chmod 755 /etc/keepalived/notify.sh"
fi
